    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          `The audit table ends up holding ${facts.swimdata.nameVariants} distinct spellings for ${facts.swimdata.clubs} clubs, and the approach self-heals a genuine data defect along the way: a status token that had leaked into a club cell produced a bogus spelling, which simply lost to the correct one on frequency. The Hebrew start list is then bridged to the English results sheet on the shared event, heat, and lane tuple, matching ${facts.swimdata.matchRate} of rows one-to-one with zero ambiguity.`,
